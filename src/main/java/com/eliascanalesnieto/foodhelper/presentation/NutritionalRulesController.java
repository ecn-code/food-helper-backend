...
@Tag(name = "Nutritional rules", description = "Configure daily nutritional limits used by planning and menus")
public class NutritionalRulesController {
    private final NutritionalRulesService service;

    @GetMapping
    @Operation(summary = "Get nutritional rules", description = "Returns the saved daily minimum and maximum for each nutrient. Unconfigured limits are null.")
    public NutritionalRulesResponse find() {
        return service.find();
    }

    @PutMapping
    @Operation(summary = "Save nutritional rules", description = "Replaces the daily nutritional limits. Each nutrient may define a minimum, a maximum, both, or neither.")
    @ApiResponses({
